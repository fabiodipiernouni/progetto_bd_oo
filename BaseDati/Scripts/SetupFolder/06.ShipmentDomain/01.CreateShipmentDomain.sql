...
create table Spedizione (
    ...
    Stato varchar2(25) default 'DaLavorare' not null, -- DaLavorare, InLavorazionePackeging, LavorataPackeging, InLavorazioneSpedizione, LavorataSpedizione
    ...
    constraint CkSpedizioneStato check (Stato in ('DaLavorare', 'InLavorazionePackeging', 'LavorataPackeging', 'InLavorazioneSpedizione', 'LavorataSpedizione')),
...
create table OrdineDiLavoroPackaging (
    Id integer generated by default as identity not null,
    DataCreazione date default sysdate not null,
    DataInizioPianificazione date, -- se valorizzato indica che l'ordine di lavoro è stato pianificato
    ...
    constraint CkOrdineDiLavoroPackagingDataInizioLavorazione check (DataInizioLavorazione is null or DataInizioLavorazione >= DataCreazione),
...
create table OrdineDiLavoroSpedizione (
    ...
    IdFiliale integer not null, -- filiale di partenza del mezzo di trasporto
    Stato varchar2(50) generated always as (case
        when IdGruppoCorriere is null then 'DaAssegnare'
        when IdGruppoCorriere is not null and DataInizioLavorazione is null then 'Assegnato'
        when IdGruppoCorriere is not null and DataInizioLavorazione is not null and DataFineLavorazione is null then 'InLavorazione'
    ...
    constraint CkOrdineDiLavoroSpedizioneDataInizioLavorazione check (DataInizioLavorazione is null or DataInizioLavorazione >= DataCreazione),
...
create table OrdineDiLavoroSpedizionePacchi (
    ...
    IdPacco integer not null,
    constraint PkOrdineDiLavoroSpedizionePacchi primary key (IdOrdineDiLavoroSpedizione, IdPacco),
